...
Juanita Duque Rosero, Sachi Hashimoto, Sachi Hashimoto, 2022

Modified hyperell_local_coord, local_analytic_interpolation, diff_D to implement local heights of D, E
when E contains at point at infinity.
...
function hyperell_local_coord(P, prec, data)
  ...
  // x "Outputs t^-2 + 5*t^4 - t^8 - 50*t^10 + O(t^12)"
  // y "Outputs t^-5 + 10*t - 2*t^5 - 75*t^7 + 50*t^11 + O(t^12)"
  ...
  // The infinite case, translated from https://github.com/jbalakrishnan/AWS/blob/master/hyperelliptic_padic_field.py
  if P`inf then
  	g := data`g;
    pol := -Evaluate(data`Q,0);
    x0:=P`x; K:=Parent(x0); Kt<t>:=LaurentSeriesRing(K,prec+2*Prime(K));
    // K = LaurentSeriesRing(self.base_ring(), name, default_prec=prec+2)
    //t = K.gen()
    L<xd> := PolynomialRing(Kt);
    //x = L.gen()
    i := 0;
    w := (xd^g/t)^2-pol;
    wprime := Derivative(w);
    if Degree(pol) eq 2*g+1 then
        xt := t^(-2);
    else
        xt := t^(-1);
    end if;
    for i in [1..50] do
        xt := xt-Evaluate(w,xt)/Evaluate(wprime,xt);
    end for;
    yt := xt^g/t;
    return xt, yt;
  end if;
...
function local_analytic_interpolation(P, Q, prec, data)
  ...
  if P`inf then
    KL<t> := LaurentSeriesRing(Parent(P`x), prec);
    xt,yt := hyperell_local_coord(P, 2*prec, data);
    g := data`g;
    td := KL!t;
    return K!(xt*td^(2*g+1)),K!(yt*td^(2*g+1)),K!(td^(2*g+1));
...
function deleteLeadingZero(pst)
  // Takes a power series element and deletes the constant coefficient if it is 0.
    if IsZero(AbsEltseq(pst)[1]) then
    newpst := Parent(pst)!0;
    t := Parent(pst).1;
    i := 1;
    coefficients := AbsEltseq(pst);
    for i in [2..#coefficients] do
      if not IsZero(coefficients[i]) then
        newpst+:=coefficients[i]*t^(i-1);
      end if;
    end for;
    return newpst;
  end if;
...
function diff_D(D, xt, yt, data) // : tiny := false, alpha := false)
  ...
  */
  ...
  function is_good(P, R)
  	if P`inf then
  		return false;
    elif P`x cmpeq R[1] and P`b[2] cmpeq -R[2] then
      return true;
  ...
  if (a cmpeq c and b cmpeq -d and is_good(P1, Z) and is_good(P2, Z) ) then
    ...
    dt := (-Derivative(xt)/(2*yt))*deleteLeadingZero(yt+d)/deleteLeadingZero(xt-c); //this is allowed because xt - c gives an exact zero
    noZero:=&+[Coefficient(dt,i)*t^i:i in [0..Degree(dt)]|not IsZero(Coefficient(dt,i))];
    if not IsWeaklyZero(dt- noZero) then
      print "Error!";
      error "Not enough precision";
    end if;
    return noZero;
  elif P2`inf then
    dt := (-Derivative(xt)/(2*yt))*(yt+b)/(xt-a);
    print "Sorry! This case has not been tested (and probably does not work)!";
    return &+[Coefficient(dt,i)*t^i:i in [0..Degree(dt)]];
  ...
  if c cmpeq Z[1] then
    ...
    forP2 := (yt+d)/(deleteLeadingZero(xt-c));
...
function res_alpha_int_beta_non_weierstrass(D1, D2, data, cycl_data : N := 0, printlevel := 0);
  ...
  */
  ...
  if pl gt 1 then print "I1 = ",integrals[1]; end if;
  if pl gt 1 then print "I2 = ",integrals[2]; end if;
...
function omega_integral(D1, D2, data : N := 0, wlcs := [**], alphas := [**], cycl_data := [**], printlevel := 0)
  ...
  */
  ...
  R_to_FR := 0;
  NRFR := N;
  BothZeroandBothInf := (IsZero(R`x) and IsZero(FR`x)) and R`inf eq true and FR`inf eq true; //Magma makes 0 precise 0 but FR at infinity is O(p^prec) making this comparison awkward
  if R`x ne FR`x  or R`b ne FR`b then
    if not BothZeroandBothInf then
      tadicprecR := 2*N+10;
      tadicprecR := 2*N;
  ...
  repeat
    try
      ...
      added_prec +:= p;
      if pl gt 1 then print "insufficient_precision", d*N+added_prec-p; "Try precision", d*N+added_prec; end if;
    end try;
  ...
  //res_alpha_int_beta_nw can be compared to res_alpha_int_beta which in Sage
  if pl gt 1 then print "sum of residues of alpha*int(beta) at non-Weierstrass points = %o.\n", res_alpha_int_beta_nw; end if;
  //
  ...
  repeat
    try
      if #wlcs eq 0 or insuff_prec then
        if pl gt 2 then print "Computing local coordinates at Weierstrass points"; end if;
      ...
      insuff_prec := true;
      weierstrass_prec +:= p;
      if pl gt 2 then print "insufficient_precision", weierstrass_prec-p; "Try precision", weierstrass_prec; end if;
    end try;
  ...
  //res_alpha_int_beta can be compared to res in Sage
